...
- [向单个用户同步推送通知](#同步推送单用户)
- [向单个用户异步推送通知](#异步推送单用户)
- [向多个用户异步批量推送通知](#异步批量推送多用户)
- [向指定标签下的用户群推送通知](#按标签推送用户群)
- [向全量用户发送推送通知](#全量推送)
...
| :----------------------------------------------------------- | :------ | :--------- | :--------------------- |
| - 接口调用：[全量推送](#全量推送)、[按标签推送](#按标签推送用户群) <br/> - 控制台创建：[全量推送任务](push_task_create.html#推送目标)、[标签推送任务](push_task_create.html#推送目标) | 计数 +1 | 计数 -1    | 执行完成后立即释放     |
| 控制台创建：[向指定用户推送的任务](push_task_create.html#推送目标)                             | 计数 +1 | 计数不变   | 统计周期结束后统一释放 |
...
| 字段          | 类型 | 描述                                                         | 是否必需 |
| :------------ | :--- | :----------------------------------------------------------- | :------- |
| `strategy`    | Int  | 推送策略：<br/> -`0`：第三方厂商通道优先，失败时走环信通道。<br/> - `1`：只走环信通道：若用户在线，则直接推送；若用户离线，消息会保留一段时间（视购买的套餐包而定），等用户上线后向其推送消息。若用户在超过消息存储期限时仍未上线，则丢弃消息。<br/> -（默认）`2`：只走第三方厂商通道：若用户离线，消息保留时间视不同厂商而定。若推送失败，直接丢弃推送消息。<br/> - `3`：在线走环信通道，离线走第三方厂商通道。如果厂商推送失败，则等待用户上线后通过环信通道下发。<br/> - `4`：只走环信通道且只推在线用户。离线用户收不到推送通知。 | 否       |
...
| `strategy`    | 否  | integer | 推送策略：<br/> -`0`：第三方厂商通道优先，失败时走环信通道。<br/> - `1`：只走环信通道：若用户在线，则直接推送；若用户离线，消息会保留一段时间（视购买的套餐包而定），等用户上线后向其推送消息。若用户在超过消息存储期限时仍未上线，则丢弃消息。<br/> -（默认）`2`：只走第三方厂商通道：若用户离线，消息保留时间视不同厂商而定。若推送失败，直接丢弃推送消息。<br/> - `3`：在线走环信通道，离线走第三方厂商通道。如果厂商推送失败，则等待用户上线后通过环信通道下发。<br/> - `4`：只走环信通道且只推在线用户。离线用户收不到推送通知。|
