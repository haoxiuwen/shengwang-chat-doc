...
const collectHeadingSlugs = (nodes: any[], result = new Set<string>()): Set<string> => {
...
const removeHeaders = (headers: any[], hiddenSlugs: Set<string>): any[] =>
  headers
    .filter((header) => !hiddenSlugs.has(normalizeSlug(header.slug ?? header.link ?? "")))
    .map((header) => ({
      ...header,
...
const syncHeaders = (): void => {
  ...
  props.headings.forEach((heading) => hiddenSlugs.add(normalizeSlug(heading)));
  if (!hiddenSlugs.size) return;

  pageData.value.headers = removeHeaders(pageData.value.headers ?? [], hiddenSlugs);
};
