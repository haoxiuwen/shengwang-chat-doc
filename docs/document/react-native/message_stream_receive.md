...
流式消息由业务服务端生成并通过 [RESTful API](/document/server-side/message_stream_send_single.html) 发送，React-Native SDK 负责接收、排序、合并和回调，不提供客户端主动发送流式消息的能力。
...
- **消息 ID**：流式消息的唯一标识，用于标识整条流式消息。在 React-Native SDK 中，可通过 `ChatMessage#msgId` 获取。
...
- 客户端能力：React-Native SDK 仅支持接收，不支持发送。
- 消息标识：`msgId` 标识整条流式消息。
...
- 已升级 SDK 至 v1.15.0 或以上版本。
- 已完成 SDK 初始化，详见 [初始化文档](initialization.html)。
- 已了解即时通讯 IM 的 [使用限制](/product/limitation.html)。

## 技术原理

流式消息由业务服务端生成，并通过即时通讯 IM 服务端按分片方式下发至客户端 SDK。React-Native SDK 在分片到达时触发消息回调，并根据分片顺序自动合并消息内容。你可结合当前分片信息、累计合并内容和传输状态实时更新界面，直到整条消息完成或异常结束。
...
1. 在 SDK 初始化完成后，注册消息监听器 `ChatMessageEventListener`。
2. 监听流式消息回调 `onStreamMessagesReceived`。当新分片到达时，SDK 会在排序后触发该回调。
3. 在回调中遍历收到的 `ChatMessage`，通过 `msgId` 定位同一条流式消息。
4. 通过 `ChatMessage#streamChunk` 获取当前分片信息。
5. 通过 `ChatTextMessageBody#content` 获取从首个分片到当前分片的累计合并内容。
6. 根据 `ChatStreamChunk#status` 更新界面展示，并在消息完成或异常结束时结束该条流式消息的渲染。
...
class ChatMessageEvent implements ChatMessageEventListener {
  onStreamMessagesReceived(messages: ChatMessage[]): void {
    console.log(`onStreamMessagesReceived: `, messages);
  }
  // 其他回调接收省略，实际开发中需要实现
}
...
```tsx
function handleStreamChunk(message: ChatMessage) {
    // 获取消息id
    const msgId = message.msgId;

    // 是否存在流式消息内容
    if (message.streamChunk === null || message.streamChunk === undefined) {
        return;
    }

    const text = message.streamChunk.text;
    const status = message.streamChunk.status;

    // 获取完整的流式内容
    fullText = '';
    if (message.body.type === ChatMessageType.TXT) {
        fullText += message.body.content;
    }

    // 获取错误码
    const errorCode = message.streamChunk.errorCode;
    
    // 获取完成原因
    const finishReason = message.streamChunk.finishReason;

    // 根据业务需要更新内容, 例如：自定义业务方法 updateStreamMessage
    // todo: 例如：更新UI
    updateStreamMessage(msgId, text, status, fullText, errorCode, finishReason);
}
```
...
通过 `ChatMessage#streamChunk` 获取 `ChatStreamChunk` 对象。该对象表示当前收到的分片信息。

| 方法 | 返回值类型 | 说明 |
| :--- | :--- | :--- |
| `text` | String | 获取当前分片的文本内容。 |
| `status` | ChatStreamChunkStatus | 获取当前分片对应的流式消息的传输状态。<br>当 `status` 返回 `COMPLETE`、`START_AND_COMPLETE` 或 `ERROR` 中任意一种状态时，表示传输完成。 |
...
| [离线推送](/document/react-native/push/push_overview.html) | 支持 | 用户离线时进行消息推送提醒。 |
...
不支持。流式消息 [仅支持通过服务端 RESTful API 发送](/document/server-side/message_stream_send_single.html)，React-Native SDK 只负责接收。

### 2. `text` 和 `content` 有何区别？
