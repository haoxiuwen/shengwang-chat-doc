...
2. 根据业务需求配置回调规则，例如，提供可访问的 HTTP 或 HTTPS 回调地址，用于接收IM 服务器发起的回调请求。详见 [规则配置说明](/product/console/basic_webhook.html#配置消息回调规则)。
...
当IM 服务器向你的回调地址发起请求后，如果收到的 HTTP 响应状态码不是 `200`，则判定本次回调失败，并立即触发一次重试。
