...
1. 在 [声网控制台](https://console.shengwang.cn/overview) [开通回调服务](callback.overview.html#开通回调服务)。
2. 根据业务需求配置回调规则，例如，提供可访问的 HTTP 或 HTTPS 回调地址，用于接收IM 服务器发起的回调请求。详见 [规则配置说明](#回调规则)。
