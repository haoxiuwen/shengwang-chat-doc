...
文本消息翻译为增值服务，要使用该功能，你需要 **联系商务经理开通**。
...
| `text`  | String | 是   | 待翻译的文本。单次请求最多支持 10,000 字符。实际翻译字数根据 **源文本字符数 × 目标语言数量** 计算，即 `text` 的字符数 x `to` 参数中的语言数量。例如，`text` 传入500 字符，`to` 传入 4 种目标语言，则则本次翻译计费字符数为 500 × 4 = 2,000。| 
