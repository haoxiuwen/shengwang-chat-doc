...
curl -X POST 'https://XXXX/app-id/{app_id}/chatrooms/{chatroomid}/white/users'  \
...
| 参数        | 类型  | 描述              |
| :---------- | :---- | :------------------- |
| `usernames` | Array | 待添加至聊天室白名单中的用户 ID，每次最多可传 100 个。|
...
| :----------- | :--- | :------------- | :----------- | :----------- |
| 400     | invalid_parameter | usernames size is more than max limit : 100 | 批量添加白名单超过上限（100）。 | 调整要添加的数量在限制（100）以下. |
