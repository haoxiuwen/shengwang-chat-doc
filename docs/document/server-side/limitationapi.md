...
| * 向 app 在线用户发送广播消息 | POST | /{org_name}/{app_name}/messages/users/broadcast | 每分钟限 1 次，每天限 50 次（可联系商务经理提升该上限）。 |     | 
| * 发送单聊流式消息                 | POST   | /{org_name}/{app_name}/stream_message/user           | 100 条/秒/App Key   | ——   | 
| * 发送群聊流式消息                 | POST   | /{org_name}/{app_name}/stream_message/chatgroup           | 100 条/秒/App Key   | ——   | 
