...
4. 若 `need_download=true`，环信服务端会主动拉取该附件，并将其保存到环信文件服务。
5. 若 `need_download=false`，附件不会自动保存到环信文件服务，后续访问仍依赖业务侧原始地址。
6. 导入成功后，客户端需按需拉取漫游消息查看结果；若附件已被保存到环信文件服务，后续也可接入 `/chatfiles/{file_uuid}` 下载链路。
...
| `msg_timestamp` | Long   | 否       | 要导入的消息的时间戳，单位为毫秒。<br/> - 不传时，环信服务器会将导入时间设为当前时间。<br/> - 不能为 `0` 或小于 `1000` 毫秒；必须大于群组创建时间，小于当前时间。<br/> - **顺序约束**：必须大于同群组中已导入的最后一条消息的时间戳，即需按时间先后顺序导入。 |
...
对于图片、语音、视频和文件等附件类型消息，**导入附件消息** 与 **发送附件消息** 的处理流程不同。导入附件消息时，可直接使用业务侧附件地址；发送附件消息时，则需要先调用 [文件上传](message_upload_file.html) 接口，再使用上传后返回的环信文件地址构造消息体。
