...
用户从第三方推送服务器获取 device token 和证书名称，然后向即时通讯服务器上传，服务器根据 device token 向用户推送消息。
...
| `notifier_name` | String | 推送证书名称。<br/> - 传入的证书名称必需与你在声网控制台的**添加推送证书**页面上填写的证书名称一致，否则推送失败。<br/> - 若 `notifier_name` 为空，表示解除当前设备与所有推送信息的绑定。 | 是       |
