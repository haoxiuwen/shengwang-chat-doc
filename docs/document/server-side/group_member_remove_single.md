...
```http
DELETE https://{host}/app-id/{app_id}/chatgroups/{group_id}/users/{username}?need_notify=false
```
...
curl -X DELETE 'https://XXXX/app-id/{app_id}/chatgroups/66XXXX85/users/user3?need_notify=false'  \
...
| 403     | exceed_limit | user XX has joined too many groups! | 用户加入的群组数达到上限。 | 退出不用的群组或在 [声网控制台](https://console.shengwang.cn/overview) 上的 **套餐包 > 预付费套餐包 > 即时通讯 IM** 上调单个用户可加入群组数上限。 |
