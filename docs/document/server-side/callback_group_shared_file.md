...
上传或删除群组共享文件后，IM 服务器会按照 [发送后回调规则](/product/console/basic_webhook.html#配置消息回调规则) 向你的 App Server 发送回调请求，App Server 可通过该回调查看上传或删除的群组共享文件，进行数据同步。
...
- [客户端上传了群组共享文件](/document/android/group_attributes.html#上传共享文件)。
- [调用 RESTful API 上传了群组共享文件](/document/server-side/group_shared_file_upload.html)。
...
- [客户端删除了群组共享文件](/document/android/group_attributes.html#管理共享文件)。
- [调用 RESTful API 删除了群组共享文件](/document/server-side/group_shared_file_delete.html)。
