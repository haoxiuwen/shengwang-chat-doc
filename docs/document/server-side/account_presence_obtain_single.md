...
curl -X GET 'https://XXXX/app-id/{app_id}/users/user1/status'   \
...
| 字段   | 类型 | 描述         |
| :----- | :--- | :---------------------- |
| `data` | JSON | 用户的在线状态数据。<br/> 格式为："用户 ID": "当前在线状态"，例如，user1 的在线和离线状态分别为 "user1": "online" 和 "user1": "offline"。 <br/> - `online`：客户端登录后和即时通讯 IM 服务器成功建立了长连接。<br/> - `offline`：iOS 和 Android 进程被杀或因网络问题断开连接，进入 `offline` 状态，此时可以接收消息的离线推送通知（前提是在声网控制台上传了推送证书，集成了离线推送服务）。 |
