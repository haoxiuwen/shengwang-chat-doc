...
```http
DELETE https://{host}/app-id/{app_id}/rest/message/roaming/chat/user/{userId}?userId={peer_userId}&msgIdList={msgIdList}&isNotify={isNotify}
```
...
curl -X DELETE 'https://XXXX/app-id/{app_id}/rest/message/roaming/chat/user/XXXX?userId=XXXX&msgIdList=XXXX&isNotify=false' \
