# 根据消息 ID 单向删除群组和聊天室的漫游消息

## 功能说明

- 根据消息 ID 单向删除指定用户的某个群组或聊天室会话的一条或多条（每次最多 50 条）漫游消息。
- 只能单向删除消息：
  - 该用户的指定漫游消息会从服务器和本地删除，该用户无法从 IM 服务端拉取到这些消息。若删除了该会话的全部漫游消息，该用户的这个会话在服务端也会被清除，拉取会话列表时拉不到该会话。
...
| `groupId` | String | 是    | 群组 ID 或聊天室 ID。                                 |
