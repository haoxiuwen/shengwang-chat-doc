...
3. 若 `need_download=true`， IM 服务端会主动拉取该附件，并将其保存到 IM 文件服务。
4. 若 `need_download=false`，附件不会自动保存到 IM 文件服务，后续访问仍依赖业务侧原始地址。
5. 导入成功后，客户端需按需拉取漫游消息查看结果；若附件已被保存到 IM 文件服务，后续也可接入 `/chatfiles/{file_uuid}` 下载链路。
...
| `msg_timestamp` | Long   | 否       | 要导入的消息的时间戳，单位为毫秒。<br/> - 不传时，IM 服务器会将导入时间设为当前时间。<br/> - 该参数不能传 `0`，也不能小于 1000 毫秒。<br/> - **顺序约束**：必须大于同单聊会话中已导入的最后一条消息的时间戳，即需按时间先后顺序导入。  |
| `need_download` | Bool   | 否       | 是否需要由IM 服务器拉取附件并保存到 IM 文件服务。<br/> - `true`：是。需确保 `body.url` 可被IM 服务器直接访问，并且返回真实有效的附件内容。<br/> - （默认）`false`：否。IM 不会主动保存该附件；此时附件访问仍依赖业务侧原始地址。|

### 附件消息导入说明

对于图片、语音、视频和文件等附件类型消息，**导入附件消息** 与 **发送附件消息** 的处理流程不同。导入附件消息时，可直接使用业务侧附件地址；发送附件消息时，则需要先调用文件上传接口，再使用上传后返回的文件地址构造消息体。
