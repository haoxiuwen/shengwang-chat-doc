...
为帮助开发者在使用 SDK 的过程中更好地落实用户个人信息保护相关要求，避免出现侵害用户个人信息权益情形，北京亿思摩博网络科技有限公司（以下简称“我们”）特制定本 环信即时通讯IM SDK合规使用说明文档（以下简称“文档”）。
...
## 二、App 使用环信即时通讯IM SDK 时的合规指引

### 1. 环信即时通讯IM SDK 所需的系统权限的说明

本SDK功能所需的权限，您可以参考如下表格，了解相关权限功能和时机。本SDK 只会检查 App 是否获得相应授权，不会主动向最终用户申请权限。

权限配置，请查阅相关配置文档：
- **Android**: [快速开始](/docs/document/android/quickstart.html)
- **iOS**: [快速开始](/docs/document/ios/quickstart.html)
- **HarmonyOS**：[快速开始](/docs/document/harmonyos/quickstart.html)
...
| 地理位置 – 前台（ohos.permission.LOCATION） | 提供发送地理位置消息的功能 | 可选 | 需要收发位置信息时 |
| 地理位置 – 后台（ohos.permission.LOCATION_IN_BACKGROUND） | 提供发送地理位置消息的功能 | 可选 | 需要收发位置信息时 |
| 地理位置 – 模糊位置（ohos.permission.APPROXIMATELY_LOCATION） | 提供发送地理位置消息的功能 | 可选 | 需要收发位置信息时 |
| 设备存储（STORE_PERSISTENT_DATA） | 持久化存储业务系统运行所需的数据 | 可选 | 需要保存业务的数据时 |
| 麦克风 | 提供发送音频消息功能 | 可选 | 需要录制音频消息时 |

本 SDK 包含推送服务等扩展业务功能，扩展业务功能涉及的权限请查阅 [即时通讯 IM SDK 接入第三方SDK清单](/docs/document/android/third_party_sdk_list.html)。
...
**SDK 功能、接口配置方式及示例说明：**
- **Android**: [接口集成](/docs/document/android/initialization.html)
- **iOS**: [接口集成](/docs/document/ios/initialization.html)
- **HarmonyOS**：[接口集成](/docs/document/harmonyos/initialization.html)

#### Android 操作系统 SDK 功能、接口配置方式及示例说明

| 系统 | 业务功能 | 相关个人信息 | 配置方式及示例 | 时机 |
| :--- | :--- | :--- | :--- | :--- |
| Android | 登录环信即时通讯 IM | 应用的应用包名、安装运行路径、设备类型、设备名称及型号、操作系统（版本信息）、网络连接类型及状态、通讯内容信息（可选） | **配置开启示例：** <br> `EMClient.getInstance().loginWithToken(mAccount, mToken, new EMCallBack() { //登录成功回调 @Override public void onSuccess() { } //登录失败回调，包含错误信息 @Override public void onError(int code, String error) { } });` <br> **配置关闭示例：** <br> `EMClient.getInstance().logout(true);` | 用户登录环信即时通讯 IM |

#### iOS 操作系统 SDK 功能、接口配置方式及示例说明

| 系统 | 业务功能 | 相关个人信息 | 配置方式及示例 | 时机 |
| :--- | :--- | :--- | :--- | :--- |
| iOS | 登录环信即时通讯 IM | 设备类型、设备名称及型号、操作系统（版本信息）、网络接入方式及状态、Bundle ID、设备识别码（APNS Token）（可选）、通讯内容信息（可选） | **配置开启示例：** <br> `EMClient.shared().login(withUsername: "userId", token: "token")` <br> **配置关闭示例：** <br> `EMClient.shared().logout(true)` | 用户登录环信即时通讯 IM |

#### HarmonyOS 操作系统 SDK 功能、接口配置方式及示例说明

| 系统 | 业务功能 | 相关个人信息 | 配置方式及示例 | 时机 |
| :--- | :--- | :--- | :--- | :--- |
| Harmony OS | 登录环信即时通讯 IM | 设备类型、设备名称及型号、操作系统（版本信息）、网络接入方式及状态、应用的应用包名、安装运行路径、通讯内容信息（可选） | **配置开启示例：** <br> `ChatClient.getInstance().loginWithToken(userId, token).then(() => {//登录成功回调}).catch((e: ChatError) => {//登录失败回调，包含错误信息});` <br> **配置关闭示例：** <br> `ChatClient.getInstance().logout().then(()=> {//success logic})` | 用户登录环信即时通讯 IM |

### 3. 可选个人信息的配置说明

对于环信即时通讯 IM SDK可选收集的个人信息的控制，您可以参考相关接入文档的相关内容，详细了解因相关信息的不收集将会对其对应的功能造成影响，您可以结合业务实际需要进行合理配置。具体配置说明文档链接

| 收集信息的具体内容 | 收集目的 | 必要或可选 | 平台 | 配置说明 |
| :--- | :--- | :--- | :--- | :--- |
| 设备识别码（APNS Token） | 为提供消息投递、多端消息同步、消息漫游功能，同时为了预防安全风险，准确识别违反法律法规的情况 | 可选 | iOS | [集成APNs推送](/docs/document/ios/push/push_apns.html) |
| 通讯内容信息 | 用于传输文字、图片、语音、视频、文件、地理位置等消息 | 可选 | Android、iOS、HarmonyOS | 调用相应的 API 才会触发对应类型消息的传输；不调用不会触发。 <br> 各消息类型的说明与代码示例详见链接文档: <br> [环信IM-Android消息类型与发送示例](/docs/document/android/message_send.html) <br> [环信IM-iOS消息类型与发送示例](/docs/document/ios/message_send.html) <br> [环信IM-HarmonyOS消息类型与发送示例](/docs/document/harmonyos/message_send.html) |
...
- **实现 SDK 功能所需权限**：网络连接状态、Wi-Fi网络状态、访问网络权限、设备存储、地理位置信息（可选）、摄像头（可选）、麦克风（可选）、保持后台运行（可选）
- **SDK 隐私政策链接**：[https://im.shengwang.cn/docs/document/android/privacy_policy.html](https://im.shengwang.cn/docs/document/android/privacy_policy.html)
...
- **SDK 使用目的和功能场景**：提供即时通讯服务功能和服务
- **SDK 涉及的个人信息类型**：设备类型、设备名称及型号、操作系统版本、网络类型及状态、应用的应用包名、安装运行路径、通讯内容信息（可选）
- **实现 SDK 功能所需权限**：访问网络权限、获取网络信息、设备存储（可选）、地理位置信息（可选）、麦克风（可选）
- **SDK 隐私政策链接**： [https://im.shengwang.cn/docs/document/android/privacy_policy.html](https://im.shengwang.cn/docs/document/android/privacy_policy.html)
...
![img](/images/android/privacy.png)
...
相关配置操作，请查阅相关配置文档：
- **Android**：[退出登录](/docs/document/android/login.html#退出登录)
- **iOS**：[退出登录](/docs/document/ios/login.html#退出登录)
- **HarmonyOS**：[退出登录](/docs/document/harmonyos/login.html#退出登录)

## 三、合规文件指引

1. [《个人信息保护法》](https://www.cac.gov.cn/2021-08/20/c_1631050028355286.htm)
...
4. [《工业和信息化部关于开展 App 侵害用户权益专项整治工作的通知》](https://www.gov.cn/)
5. [《App 违法违规收集使用个人信息行为认定方法》](https://www.cac.gov.cn/2019-12/27/c_1578986455686625.htm)
...
8. [《GB/T 35273-2020信息安全技术个人信息安全规范》](https://openstd.samr.gov.cn/bzgk/std/newGbInfo?hcno=4568F276E0F8346EB0FBA097AA0CE05E&refer=outter)
9. [《网络安全标准实践指南—移动互联网应用程序（App）使用软件开发工具包（SDK）安全指引》](https://www.tc260.org.cn/)
