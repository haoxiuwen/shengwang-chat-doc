...
| [初始化 SDK](/document/ios/initialization.html)         | 使用 IM 的各项功能前，必须先初始化。传入你应用的 App Key 进行 [初始化](/document/ios/initialization.html)。 初始化时，可配置加群、退群推送等重要特性。|
| [登录 IM](/document/ios/login.html)       | 使用创建的用户登录 IM。登录成功后，你可以使用 IM 的功能。 |
