...
```csharp
// 设置设备名称并进行初始化
Options options = new Options("YourappId");
...
2. 初始化 SDK 时，调用 `Options#CustomOSType` 方法自定义设置登录设备的平台。确保该方法中的 `platform` 参数的值与 [声网控制台](https://console.shengwang.cn/overview) 的自定义设备平台的值相同。
