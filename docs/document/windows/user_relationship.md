...
- 详见 [环信控制台文档](/product/console/basic_user.html#用户黑名单)。
...
public class ContactManagerDelegate : IContactManagerDelegate {
    // 当前用户新增了好友。用户 B 向用户 A 发送好友请求，用户 A 同意该请求，用户 B 收到 `onContactAgreed` 事件，双方用户收到 `OnContactAdded` 事件。
...
调用 `DeleteContact` 删除指定好友。被删除的用户收到 `OnContactDeleted` 回调。删除好友时会同时删除对方好友列表中的该用户，建议执行双重确认，以免发生误删操作。删除操作不需要对方同意或者拒绝。
