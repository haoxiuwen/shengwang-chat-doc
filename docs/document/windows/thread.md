...
本文介绍如何使用即时通讯 IM SDK 在实时互动 app 中创建和管理消息话题，并实现消息话题相关功能。

如需查看消息相关内容，参见 [消息话题中的消息管理](thread_message.html)。

## 技术原理

即时通讯 IM Windows SDK 提供 `IChatThreadManager`、`ChatThread`、`ChatThreadEvent` 和 `IChatThreadManagerDelegate` 类，用于管理消息话题，支持你通过调用 API 在项目中实现如下功能：
