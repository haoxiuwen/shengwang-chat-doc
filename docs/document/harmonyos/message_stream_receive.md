...
流式消息由业务服务端生成并通过 [RESTful API](/document/server-side/message_stream_send_single.html) 发送，HarmonyOS SDK 负责接收、排序、合并和回调，不提供客户端主动发送流式消息的能力。
...
- 客户端能力：HarmonyOS SDK 仅支持接收，不支持发送。
- 消息标识：`msgId` 标识整条流式消息。
...
流式消息由业务服务端生成，并通过即时通讯 IM 服务端按分片方式下发至客户端 SDK。HarmonyOS SDK 在分片到达时触发消息回调，并根据分片顺序自动合并消息内容。你可结合当前分片信息、累计合并内容和传输状态实时更新界面，直到整条消息完成或异常结束。
...
1. 在 SDK 初始化完成后，注册消息监听器 `ChatMessageListener`。
2. 监听流式消息回调 `onStreamMessageReceived`。当新分片到达时，SDK 会在排序后触发该回调。
3. 在回调中遍历收到的 `ChatMessage`，通过 `msgId` 定位同一条流式消息。
4. 通过 `ChatMessage#getStreamChunk()` 获取当前分片信息。
5. 通过 `TextMessageBody#getContent()` 获取从首个分片到当前分片的累计合并内容。
...
通过 `ChatMessage#getStreamChunk()` 获取 `ChatStreamChunk` 对象。该对象表示当前收到的分片信息。
...
`TextMessageBody#getContent()` 用于获取从首个分片到当前分片的累计合并内容。
...
function handleStreamChunk(message: ChatMessage): void {
  ...
  const body = message.getBody();
  if (body instanceof TextMessageBody) {
    const currentFullText: string = body.getContent();
  }
}
```
