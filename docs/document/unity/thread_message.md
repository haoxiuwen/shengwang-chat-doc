...
消息话题中的消息消息类型属于群聊消息类型，与普通群组消息的区别是需要添加 `IsThread` 标记。使用消息话题中的消息功能前，你需联系商务开通。

本文介绍即时通讯 IM Unity SDK 如何发送、接收以及撤回消息话题中的消息。

## 技术原理

即时通讯 IM Unity SDK 提供 `IChatManager`、`Message` 和 `IChatThreadManager` 类，用于管理消息话题中的消息，支持你通过调用 API 在项目中实现发送、接收、撤回和获取消息话题中的消息。
...
- 已集成即时通讯 IM `(1.0.5 或以上版本)` 的基本功能，账户登录成功。
