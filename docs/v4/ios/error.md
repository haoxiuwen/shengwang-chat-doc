...
| 221    |      EMErrorUserNotOnRoster   | 非好友禁止发消息：开通非好友禁止发消息后，非好友间发消息提示此错误。你可以在 [声网控制台](https://console.shengwang.cn/overview) 的 **功能配置 > 基础功能** > **用户** 开启好友关系检查功能。 | 需要先调用 `EMContactManager#addContact` 方法添加好友。对方同意好友请求后，才能发送消息。 |
...
| 353    | EMError#CONNECTION_STREAM_CLOSED  | 连接服务器时流被关闭。 | 请先检查设备网络状态。若网络连通正常，建议稍等片刻后重新尝试登录。 |
...
| 504    |    EMErrorMessageRecallTimeLimit    | 消息撤回超时错误：消息撤回超过时间限制时会提示该错误。 | 可在 UI 上进行错误提示，或者在 [声网控制台](https://console.shengwang.cn/overview) 延长消息可撤回时间，最多可设置为 7 天。 |
...
| 1001   |      EMErrorContactReachLimit       | 邀请者的好友数量已达到上限。       | 可以在 UI 上提示该错误，或在声网控制台提升用户的好友数上限。 |
| 1002   |    EMErrorContactReachLimitPeer     | 受邀请者的好友数量已达到上限。   | 可以在 UI 上提示该错误，或在声网控制台提升用户的好友数上限。 |
| 1100   |     EMErrorPresenceParamExceed      | - 用户在线状态订阅功能 Presence 未开通。<br/> - 参数长度超出限制：调用 Presence 相关方法时参数长度超出限制。 | <br/>使用 Presence 功能前需要在声网控制台开通。 <br/> - 调用[发布自定义在线状态 API](presence.html#发布自定义在线状态) 时设置的在线状态详细信息的长度不能超过 64 字节。 |
