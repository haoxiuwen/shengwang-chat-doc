...
- SDK 4.13.0 及之后的版本支持对单聊、群组和聊天室会话中各类消息进行编辑：
 - 文本消息：支持编辑消息内容字段 `msg` 和扩展字段 `ext`。
 - 自定义消息：支持编辑 `customEvent` 、 `customExts` 和 扩展字段 `ext`。
 - 图片/语音/视频/文件/位置/合并消息：仅支持编辑扩展字段 `ext`。
 - 透传消息：不支持编辑。

## 技术原理

即时通讯 IM 通过 `modifyMessage` 方法实现消息编辑。
...
你可以调用 `modifyMessage` 方法编辑已经发送成功的消息。除消息体和消息扩展属性 `ext` 外，该消息的其他信息（例如，消息 ID、消息发送方、接收方）均不会发生变化。

SDK 4.13.0 及以后版本中编辑消息成功后会返回 `modifiedInfo` 字段。你可以从`res.modifiedInfo` 中获取消息的最新编辑时间（`operationTime`）、最新编辑的操作者（`operatorId`）以及消息编辑次数（`operationCount`）。

消息编辑后，消息的接收方会收到 `onModifiedMessage` 事件，该事件中会携带编辑后的消息对象、最新一次编辑消息的用户以及消息的最新编辑时间。对于群组和聊天室会话，除了编辑消息的用户，群组/聊天室内的其他成员均会收到该事件。

:::tip
1. 一条消息默认最多可编辑 10 次。
2. 若通过 RESTful API 编辑自定义消息，消息的接收方也通过 `onModifiedMessage` 事件接收编辑后的自定义消息。
:::
...
// 自定义消息支持编辑 `customEvent`、`customExts` 和 `ext` 字段
...
// 图片/语音/视频/文件/位置/合并消息只支持编辑 `ext` 字段
