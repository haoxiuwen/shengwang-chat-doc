...
- 若使用**群组和聊天室**的消息置顶功能，你需要将 IM SDK 升级至 4.6.0 版本并联系商务经理开通。
...
const options = {
   ...
   pageSize: 20,
   // 开始获取数据的游标位置。首次调用方法时传 `null` 、空字符串（''）或不传该字段。后续调用传入上一次查询结果的游标 res.data.cursor，若 cursor 的值为空字符串（''），表示当前为最后一页数据。
   cursor: ''
}
