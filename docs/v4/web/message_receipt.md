...
- 单聊消息送达回执：
  ...
  1. SDK 初始化时，用户将 `ConnectionParameters` 类型中的 `delivery` 参数设置为 `true`。 
...
- 要使用群消息已读回执功能，你需要在 [声网控制台](https://console.shengwang.cn/overview) 将即时通讯 IM 升级至旗舰版。
...
| 使用限制 | 默认 | 描述 |
| :--------- | :----- | :------- | 
| 功能开通   | 关闭   | 若要使用该功能，你需要在 [声网控制台](https://console.shengwang.cn/overview) 将即时通讯 IM 升级至旗舰版。若使用前未开通，SDK 提示 503 "group ack not open" 错误。| 
