...
2. [**使用默认推送模板**](push_template.html#设置推送模板)：若有默认模板 `default`，发消息时无需指定。
...
5. [**使用自定义推送模板**](push_template.html#设置推送模板)：发送消息时通过消息扩展字段指定推送模板名称。

:::tip
推送模板为离线推送的高级功能，使用前需要 [在声网控制台开启](push_template.html#开通服务)。
:::