...
除了满足用户离线条件外，要使用第三方离线推送，你还需在[环信控制台](https://console.easemob.com/user/login)配置推送证书信息，例如，对于华为推送，需配置**证书名称**和**推送密钥**，并调用客户端 SDK 提供的 API 向IM 服务器上传 device token。
...
关于如何获取推送 Token 并上传至IM 服务器，详见各推送服务的集成文档，例如 对于 FCM 推送，请参阅 [FCM 推送集成文档](/v4/android/push/push_fcm.html#步骤四-fcm-推送集成)。
...
SDK 内部会按照以上顺序检测设备的推送支持情况。如果未设置第三方推送或者不满足使用第三方推送的条件，即时通讯 IM SDK 会通过一些保活手段尽可能的保持与IM 服务器的长连接，以确保消息及时送达。
