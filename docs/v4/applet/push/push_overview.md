...
除了满足用户离线条件外，要使用第三方离线推送，用户还需在声网控制台配置推送证书信息，例如，对于华为推送，需配置 **证书名称** 和 **推送密钥**，并调用客户端小程序 SDK 提供的 `uploadPushToken` 方法向IM 服务器上传 device token。
...
**即时通讯 IM 小程序 SDK 本身不支持离线推送，只支持对移动端离线推送进行如下配置**：
