...
支持 [服务端消息搜索](/value-added/search/message_search_web.html)，可根据关键词组合、会话 ID、消息类型、时间范围及消息内容或扩展属性进行筛选。该功能需联系商务经理开通后方可使用。
...
## 4.19.0 Dev 2026-02-04（开发版）

#### 新增特性

[uniapp 平台支持自动登录](uniapp.html#自动登录)。

- 初始化时将 `isAutoLogin` 选项设置为 `true`，启用自动登录。
- 新增 [错误码 214 和 220](error.html)，在自动登录场景中用户登录设备数超过限制或用户的登录设备与上次不一致时报错。
...
- [IM SDK] uni-app SDK 支持 [uni-app 推送插件](/v4/applet/push/uniapp_push.html)。
...
## v4.8.1 Dev 2024-07-17（开发版）

#### 新增特性

- [IM SDK] 新增[日志上报](initialization.html#日志上报)功能, 即将日志会上传到 IM 服务器。该功能默认关闭，如有需要, 可联系商务开通。
...
- [IM SDK] [getJoinedGroups](/v4/web/group_manage.html#获取群组列表) 方法中增加请求参数支持返回群组成员人数和自己的角色。
...
- [IM SDK] 新增 [getConversationList](/v4/web/conversation_list.html) 方法解析会话中的最新一条消息；
