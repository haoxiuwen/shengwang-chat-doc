...
| 只给好友发送消息  | 只能给好友发送消息。你可以联系声网商务开启该功能。 |
...
对于单个 app，发送单聊消息 REST API 默认存在以下限制（你可以联系商务经理上调限制）：
...
默认情况下，SDK 对单个用户发送消息的频率未做限制。如果你联系了商务经理设置了该限制，一旦在单聊中单个用户的消息发送频率超过设定的上限，SDK 会上报错误，例如 Android 端的错误码为 509 `MESSAGE_CURRENT_LIMITING`。
